function [xhat, meas] = phone_filter(calAcc, calGyr, calMag)
    ...
    while server.status()  % Repeat while data is available
      ...
      if ~any(isnan(acc))  % Acc measurements are available.
         if  abs(acc) < 12 %??? Don't know if this is correct, tuning I guess
            ...
            ownView.setAccDist(0) %As long as Zero, not disturbed
         else
            %Skip update if meas is outlier
            ownView.setAccDist(1)%When 1, disturbed
        end
      end
      gyr = data(1, 5:7)';
      if ~any(isnan(gyr))  % Gyro measurements are available.
        ...
        [x, P] = tu_qw_no_omega(x, P, Q);
        [x, P] = mu_normalizeQ(x, Q);
      end
      ...
      if ~any(isnan(mag))  % Mag measurements are available.
        ...
        if  abs(L - norm(mag)) < L*0.1 %??? Don't know if this is correct, tuning I guess2
            ...
            [x, P] = mu_normalizeQ(x, P);
            ownView.setMagDist(0) %As long as Zero, not disturbed
        else
            %Skip update if meas is outlier
